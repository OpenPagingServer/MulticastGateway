...
def show_server_list(store):
    ...
    for row in rows:
        ...
        public_key = normalize_public_key(row.get("public_key"))
        host = normalize_host(row.get("host"))
        port = row.get("port")
        status = peer_status_from_timestamp(row.get("last_seen"), now=now_value)
        print(label)
        if host:
            print(f"IPaddr: {host_port_for_display(host, port)}")
            print(f"Status: {status}")
            print(f"PublicKey: {public_key}")
        else:
            print(f"Status: {status}")
            print(f"Online IP: {row.get('last_ip') or 'Unknown'}")
            print(f"PublicKey: {public_key}")
        print("")


def add_server_manually(store):
    print("Please specify which role this gateway has.")
    print("1.) This gateway will connect to another gateway or to Open Paging Server")
    print("2.) Another gateway will connect to this one")
    print("")
    role = prompt("Selection: ").strip()
    if role not in {"1", "2"}:
    ...
    if role == "1":
        ...
        public_key = prompt("Public key: ").strip()
        peer_key = normalize_public_key(public_key)
        store.upsert_peer(label or peer_key, "", DEFAULT_PORT, peer_key, peer_type="gateway", enabled=1)
    print("Server added.")
    print("")
...
def is_gateway_runtime_running():
    check_lock = FileInstanceLock(default_gateway_lock_path(BASE_DIR))
    if not check_lock.acquire():
        return True
    check_lock.close()
    return False
...
def run_gateway(review=False):
    ...
    if review:
        if not is_gateway_runtime_running():
            print("Multicast Gateway must already be running before using -r.")
            print("")
            return 1
        console_menu(store, identity)
        return 0
